...
class Alphabet extends FlxSpriteGroup
{
	...
	override function update(elapsed:Float)
	{
		if (isMenuItem)
		{
			if(changeX)
			{
				if (isFreeplayVersion)
				{
					var scaledY = FlxMath.remapToRange(targetY, 0, 1, 0, 1.3);
					x = FlxMath.lerp(x, Math.exp(Math.abs(scaledY * 0.6)) * -40 + (FlxG.width * 0.15), GlobalFreeplayStuff.lerpVal);
					if (x < -900)
						x = -900;
				}
				else
					x = FlxMath.lerp(x, (targetY * distancePerItem.x) + startPosition.x, GlobalFreeplayStuff.lerpVal);
			}
			if(changeY)
