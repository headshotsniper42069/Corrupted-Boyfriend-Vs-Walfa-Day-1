...
uniform bool canGlitch = true;

// https://www.shadertoy.com/view/MltBzf

float rand(vec2 p)
{
    float t = floor(iTime * 35.) / 10.;
...
void mainImage()
{
	...
	{
		float colR = texture(iChannel0, vec2(uv.x, uv.y)).r;
        float colG = texture(iChannel0, vec2(uv.x, uv.y)).g;
        float colB = texture(iChannel0, vec2(uv.x, uv.y)).b;
